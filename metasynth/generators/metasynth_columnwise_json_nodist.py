...
"""
    ...
    def _generate_batch(self, num=1, allowed_dropout=0.2):    
        ...
        if self.chat is None:
            successful_chats = [[] for _ in range(num * max(1, min(int(len(groups) / 2), 3)))]
        else:
            successful_chats = [self.chat for _ in range(num * max(1, min(int(len(groups) / 2), 3)))]
            ...
            while (len(current_chats) / max(1, (len(current_chats) + len(successful_chats)))) > allowed_dropout:
                ...
                for _ in range(3):
                    try:
                        ...
                        responses = asyncio.run(get_responses(current_chats, llm=self.llm, sampling_params=self.sampling_params, client=self.client, llm_name=self.model_name, temperature=self.temperature))
                        break
                    except:
                        print("Retrying")
                        continue
